...
export function EditableReview({ editor, busy }: { editor: InspectionEditor; busy: boolean }) {
  ...
  return (
    <fieldset disabled={busy} className="flex min-w-0 flex-col gap-3">
      ...
      <div ref={attachList} className="flex max-h-[30dvh] flex-col gap-3 overflow-y-auto">
        {state.review.annotations.map((annotation, index) => (
          <div
            key={annotation.id}
            data-region-id={annotation.id}
            className={`flex min-w-0 flex-col gap-3 rounded-md border p-3 ${state.selected === annotation.id ? 'border-emerald-600 bg-emerald-500/10 ring-1 ring-emerald-600' : ''}`}
          >
            <IconButton
              ...
              aria-pressed={state.selected === annotation.id}
              variant="ghost"
              size="sm"
              disabled={busy}
