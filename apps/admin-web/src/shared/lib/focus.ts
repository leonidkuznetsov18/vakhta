...
export function focusFirstField(container: HTMLElement | null): void {
  ...
  const field = container.querySelector<HTMLElement>(
    'input:not([type="hidden"]):not([disabled]):not([readonly]):not([aria-hidden="true"]), select:not([disabled]):not([aria-hidden="true"]), textarea:not([disabled]), button[role="combobox"]:not([disabled])',
  );
  const target =
    field ??
