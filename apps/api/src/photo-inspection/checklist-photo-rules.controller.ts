...
@Controller('admin/checklists/:definitionId/photo-rules')
...
export class ChecklistPhotoRulesController {
  ...
  @Get()
  get(@Param('definitionId', ParseUUIDPipe) definitionId: string, @CurrentUser() user: WebUser) {
    return this.rules.get(definitionId, user);
  }
  @Put()
  ...
  save(
    ...
    return this.rules.save(definitionId, input, user);
  }
}
