...
- **Browser E2E:** add one maintained browser runner after checking existing tooling. A small Playwright
  ...
  belongs to a follow-up PR. There is currently no `test:e2e` command to run.
...
acceptance criteria and owns its test worktree; the integration owner reviews the combined revision.
...
Add coverage reporting in a small PR with the Vitest-compatible provider version, record line/branch
