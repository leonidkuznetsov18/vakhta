...
The `apps/api/src/handover` module implements spec 5.6-5.9 (FR-CLN, FR-PHO, FR-HND). `CLEANING_REMINDER_MINUTES` before the planned end the worker reminds about cleaning with a "…start cleaning" button; `CLEANING_DONE` opens a report draft with the checklist bound to the employee position (schedule assignment, otherwise the current position), refined by the zone type when the position has several; admins build checklists in the panel tab "Checklists" (`ChecklistsService`, ADR-0012); a checklist is bound to one or more positions through `checklist_definition_positions`, and the employee card attaches an existing checklist to the position (`POST /admin/org/checklists/:id/positions`). A position without a checklist gets no draft: the bot hides the checklist button (`ShiftScreenView.checklistAvailable`) and `SUBMIT_HANDOVER` needs no report; the employees table shows the checklist of every position. A checklist is an ordered list of `CHECK`, `NOTE` and `PHOTO` items and must contain at least one photo item. In the bot: ✅ or ⚠️ per check (a remark needs a category from `reason_codes` kind `HANDOVER`, text, a safety assessment and the need for master/cleaning/repair), a message to the next shift, a 📷 button per photo item (a repeated photo of an item replaces the previous one), "I cannot finish cleaning" with a reason (FR-CLN-05). "Submit report" validates the draft with the domain function `validateHandoverDraft`, moves the report to `SUBMITTED` with the acceptance deadline `acceptDeadline` and performs the `SUBMIT_HANDOVER` transition in the same transaction; the handing employee closes the shift without waiting for the receiver (FR-HND-02). The zone is optional on a report: a shift without a zone (no schedule, or a master start without a zone) still gets its checklist, its report has no receiver and is escalated to the master at submission; when a checklist applies, `SUBMIT_HANDOVER` needs a submitted report unless the master overrides it. `MasterStartShiftCommand.zoneId` lets the master pick the zone when no assignment provides one. `CONTINUE_WORK` after the report makes it `SUPERSEDED` (FR-HND-07).
...
A manual adjustment (`POST /admin/bonus/scores/:id/adjust`) has a reason from the `ADJUSTMENT` directory, a comment and an author; without a `criterion` it applies to the shift score as a whole (a plain bonus or penalty, capped at 0–100), otherwise to one criterion. It can be edited (`PATCH /admin/bonus/adjustments/:id`) and withdrawn (`DELETE`, status `CANCELLED`) while the period is open; a reduction above `secondApprovalThreshold` waits for a second approval by another person. A shift the rules cannot score (`MANUAL_REVIEW`) is settled by `POST /admin/bonus/scores/:id/review`: the master sets a score (the default is the share of points earned) or excludes the shift; the employee is notified of adjustments and reviews (`BONUS_ADJUSTED`, `BONUS_REVIEWED`). The panel shows a month summary, a rating of the best employees and one card per employee with these actions. Closing a period (`POST /admin/bonus/period/:siteId/:month/close`) pins the rules version, confirms evaluated shifts, computes `S_month` per formula 7.6 and notifies employees; HR sets the bonus base, accounting exports CSV only for a closed period with an audit row (FR-WEB-04/05). After closing, scores are not rewritten; an adjustment remains a separate entry (FR-COR-05). In the bot "My scores": the month coefficient, shifts with statuses, a breakdown of every criterion with its basis and "Appeal" within `APPEAL_WINDOW_DAYS` (the appeal goes to the unit manager as an `APPEAL` request).
...
- CI/CD: every push to `master` runs the checks, `semantic-release` bumps the version and writes `CHANGELOG.md` from Conventional Commits, images go to GHCR and the panel and kiosk to Cloudflare Pages; Railway redeploys the API and worker after the check suite passes. Details in `docs/deploy.md`. Every published release is also announced in the Telegram group (job `announce`, `scripts/release/announce-telegram.mjs`; see docs/deploy.md). A support assistant bot (`apps/api/src/support`, docs/features/12-support-bot.md) answers questions about the system from `docs/features/*.md`, the user guide and the changelog through Claude, with optional voice through OpenAI audio; the worker bot links to it with the "Support" button.
