...
- CI/CD: every push to `master` runs the checks, `semantic-release` bumps the version and writes `CHANGELOG.md` from Conventional Commits, images go to GHCR and the panel and kiosk to Cloudflare Pages; Railway redeploys the API and worker after the check suite passes. Details in `docs/deploy.md`.
