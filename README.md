...
The `apps/api/src/handover` module implements spec 5.6-5.9 (FR-CLN, FR-PHO, FR-HND). `CLEANING_REMINDER_MINUTES` before the planned end the worker reminds about cleaning with a "…start cleaning" button; `CLEANING_DONE` opens a report draft with the checklist that fits the employee position (schedule assignment, otherwise the current position) and the zone type; admins build checklists in the panel tab "Checklists" (`ChecklistsService`, ADR-0012), and the default with the eight spec items plus three photo angles is created automatically when none is defined. A checklist is an ordered list of `CHECK`, `NOTE` and `PHOTO` items and must contain at least one photo item. In the bot: ✅ or ⚠️ per check (a remark needs a category from `reason_codes` kind `HANDOVER`, text, a safety assessment and the need for master/cleaning/repair), a message to the next shift, a 📷 button per photo item (a repeated photo of an item replaces the previous one), "I cannot finish cleaning" with a reason (FR-CLN-05). "Submit report" validates the draft with the domain function `validateHandoverDraft`, moves the report to `SUBMITTED` with the acceptance deadline `acceptDeadline` and performs the `SUBMIT_HANDOVER` transition in the same transaction; the handing employee closes the shift without waiting for the receiver (FR-HND-02). The zone is optional on a report: a shift without a zone (no schedule, or a master start without a zone) still gets its checklist, its report has no receiver and is escalated to the master at submission, and `SUBMIT_HANDOVER` needs a submitted report for every shift unless the master overrides it. `MasterStartShiftCommand.zoneId` lets the master pick the zone when no assignment provides one. `CONTINUE_WORK` after the report makes it `SUPERSEDED` (FR-HND-07).
