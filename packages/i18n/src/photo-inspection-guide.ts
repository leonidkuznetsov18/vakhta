...
export const inspectionGuideUk: SectionGuide = {
  ...
  steps: [
    ...
    'Якщо бачите обʼєкт, оберіть «Прямокутник» і обведіть його, а для предмета неправильної форми «Багатокутник»: натискайте на кути і замкніть контур. У «Що позначено» оберіть обʼєкт з чекліста або з каталогу. Одна область — один предмет. Потім оберіть значення: «Порушення», «Дозволено тут» або «Не впевнений». «Додати деталі» потрібне лише для додаткового пояснення.',
...
export const inspectionGuideEn: SectionGuide = {
  ...
  steps: [
    ...
    'For a visible object, choose Rectangle and drag around it, or Polygon for an irregular shape: click its corners and close the outline. In What is marked, choose the object from the checklist or the catalog. One object per region. Then choose its meaning: Violation, Allowed here or Unsure. Use Add details only for extra context.',
...
export const inspectionGuideRu: SectionGuide = {
  ...
  steps: [
    ...
    'Если видите объект, выберите «Прямоугольник» и обведите его, а для предмета неправильной формы «Многоугольник»: нажимайте на углы и замкните контур. В «Что отмечено» выберите объект из чек-листа или из каталога. Одна область — один предмет. Затем выберите значение: «Нарушение», «Разрешено здесь» или «Не уверен». «Добавить детали» нужно только для дополнительного пояснения.',
