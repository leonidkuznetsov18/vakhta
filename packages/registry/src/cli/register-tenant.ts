...
try {
  const tenant = await registerExistingTenant(db, new SecretCipher(keyHex), {
    ...
    databaseUrl: required(
      'database-url',
      values['database-url'] ?? process.env['TENANT_DATABASE_URL'],
    ),
    ...
    botToken: values['bot-token'] ?? process.env['TENANT_BOT_TOKEN'],
    botUsername: values['bot-username'],
    webhookSecret: values['webhook-secret'] ?? process.env['TENANT_WEBHOOK_SECRET'],
