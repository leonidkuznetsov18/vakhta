...
- [ ] T031 (API module guard, disabled-bot webhook drop, polling stop and the bot greeting are done; panel/kiosk gating, `MeView.tenant` and branding surfaces remain, with branding in progress in a parallel session) Module guard on kiosk, terminal, activation, relink and webhook routes; `MeView.tenant`; panel navigation and action gating with tooltips; kiosk notice screen; bot greeting with the display name; i18n in three catalogs.
