...
| `employee_compensation_entries` (`id`, `employee_id`, `effective_from date`, `employment_rate numeric`, `hourly_rate numeric`, `monthly_salary numeric`, `currency text default 'UAH'`, `corrects_entry_id uuid` self-FK, `reason text`, `created_by uuid`, `created_at timestamptz`) | `employment_rate > 0 and <= 1`; scale <= 2 for all numbers; amounts `>= 0 and < 10000000000`; `hourly_rate is not null or monthly_salary is not null`; `currency = 'UAH'`; `corrects_entry_id is null or reason is not null`; correction's `effective_from` equals the corrected entry's (trigger); append-only trigger; index `(employee_id, effective_from, created_at)` |

`employee_positions.manager_employee_id` snapshots the designated master when a new assignment is written.
History renders that recorded person; legacy nulls remain unknown rather than inventing a past master.
...
| `PUT` / `DELETE /admin/employees/:id/avatar`                | ADMIN, HR                                                     | multipart ≤10 MB; magic-byte type check; server normalizes to 512 px square WebP, strips metadata, applies orientation; stores private object, swaps reference and audits in one transaction; old object expires after commit for the avatar cleanup service; staged writes also expire if linking fails |
...
- `ui/`: `ProfilePage`, `ProfileHeader`, `ContactsSection`, `PersonalSection`, `WorkSection`,
  `ScheduleSection`, `CompensationSection`, one profile Edit action for all personal fields; read-only directory Sheet (full width on mobile), avatar
  upload control reusing `components/app/avatar.tsx`. Sections absent when access flags deny.
- Route `administration/employees/:employeeId`; `EmployeesTab.tsx` names link to it and keep list
  filters, page and scroll. Rows open a read-only Sheet with a profile deep link; disclosure icons and
  expanded editors are removed by the owner refinement on 2026-09-13.
...
avatar writes require the employee version and serialize reference swaps. Staged media expires durably;
cleanup retries per object and rechecks references before deleting. Storage and database are not atomic.
